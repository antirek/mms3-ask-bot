...
export class MessageHandler {
  ...
  async handleIncomingMessage(update) {
    try {
      ...
      // Классифицируем намерение через AI с файлами
      const intents = getAllIntents();
      const intentResult = await this.aiClassifier.classifyIntentWithFiles(
        userMessage, 
        contextMessages, 
        intents,
        fileIds
  ...
  buildSuccessResponse(intentResult) {
    // Если AI вернул готовый ответ (из файлов или на основе намерения), используем его
    if (intentResult.answer && intentResult.answer.trim()) {
      console.log('Используется ответ от AI:', intentResult.answer);
    ...
    // Базовые ответы для разных намерений (fallback, если AI не вернул answer)
    const responses = {
